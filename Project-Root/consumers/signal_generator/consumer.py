...
async def main():
    ...
    consumer = KafkaConsumerService(
        ...
        group_id="signal-generator-group",
        bootstrap_servers=config.KAFKA_BOOTSTRAP_SERVERS,
        message_handler=handler,
        dlt_topic=config.DEAD_LETTER_TOPIC_SIGNAL,
    ...
    try:
        ...
        logger.info("Shutting down signal generator resources...")
        await shutdown_kafka_producer(producer)
        await shutdown_kafka_producer(dlt_producer)
        await close_pool()

        logger.info("Signal generator pipeline shutdown complete.")
